    ...
    if (fixed.current && j1.current && j2.current && j3.current && card.current && band.current) {
      [j1, j2].forEach(ref => {
        ...
        const clampedDistance = Math.max(
          0.1,
          Math.min(1, ref.current.lerped.distanceTo(currentTrans))
        );
      ...
      // Correctly handle rotation to prevent physics explosion (NaNs)
      const angVel = card.current.angvel();
      const rotQuat = card.current.rotation();
      // Use a temporary euler to get Y rotation safely
